...
Vec<ZZ_p> hypersurface::frob_J( const int64_t coordinate, const int64_t N)
{
    return frob_J_ZZ(coordinate, N, 0);
}
Vec<ZZ_p> hypersurface::frob_J_ZZ( const int64_t coordinate, const int64_t N,const int64_t loop)
{
    assert(N > 0);
    int64_t e, i, j, m, sum, end, val;
    ZZ_p fact;
    //ZZ factZZ;
    ZZ tmp, remainder;
    ZZ Djm, bin;
    ...
    Vec<ZZ> G, Gswap;
    Vec<ZZ_p> F, G_ZZ_p, G0, H_last, *H_temp;
    Mat<ZZ> Mred;
    ...
    {   
        ...
        if( e >= m )
        {
            ...
            for( i = j ; i < N ; i++)
            {
                bin = binomial_ZZ(m + i - 1, m - 1) * binomial(i,j);
                if(j%2 == 0)
                    Djm += bin;
                else
                    Djm -= bin;
        ...
        for( Hit = H.begin(); Hit != H.end(); Hit++)
        {
            ...
            switch(loop)
            {
                case 0:
                    ...
                    if( end > d * n - n )
                    {
                        ...
                        dR->reduce_vector_J_poly_ZZ(G, dest, v, end - (d * n - n), G);
                        i = end - (d*n - n);
                    }
                    for( ; i < end; i++)
                    {
                ...
                case 2:
                    for(i = 0; i < end; i++)
                    {
                        ...
                        dest = tweak(p * Hit->first - (i + 1) * v, d * n - n);
                        y = src - dest;
                        Mred = dR->get_reduction_matrix_J_ZZ(dest, y);
                        mul(G, Mred, G);
                        for( j = 0; j < (int64_t) G.length(); j++)
                            rem(G[j], G[j], ZZ_p::modulus() );
                    }
                    break;
            }
            ...
            if(! IsZero(G_ZZ_p) )
            {
                ...
                else
                {
                    ...
                    mul(G_ZZ_p, *(dR->get_inclusion_matrix_J(dest)), G_ZZ_p);
